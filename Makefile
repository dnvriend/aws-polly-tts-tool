...
typecheck: ## Run type checking with mypy
	uv run mypy aws_polly_tts_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r aws_polly_tts_tool -c pyproject.toml
...
run: ## Run aws-polly-tts-tool (usage: make run ARGS="...")
...
uninstall-global: ## Uninstall global installation
	uv tool uninstall aws-polly-tts-tool
